...
namespace Bot_Dofus_1._29._1.Interfaces
{
    public partial class UI_Mapa : UserControl
    {
        ...
        private void UI_Mapa_Load(object sender, EventArgs e)
        {
            ...
            cuenta.game.mapa.mapa_actualizado += get_Eventos_Mapa_Cambiado;
            cuenta.game.mapa.entidades_actualizadas += () => control_mapa.Invalidate();
            cuenta.game.personaje.movimiento_pathfinding_minimapa += get_Dibujar_Pathfinding;
        }
        ...
        {
            Mapa mapa = cuenta.game.mapa;
            Celda celda_actual = cuenta.game.personaje.celda, celda_destino = mapa.get_Celda_Id(celda.id);

            if (botones == MouseButtons.Left && celda_actual.id != 0 && celda_destino.id != 0 && !abajo)
            {
                switch (cuenta.game.manejador.movimientos.get_Mover_A_Celda(celda_destino, mapa.celdas_ocupadas()))
                {
        ...
        private void get_Dibujar_Pathfinding(List<Celda> lista_celdas) => Task.Run(() => control_mapa.agregar_Animacion(cuenta.game.personaje.id, lista_celdas, PathFinderUtil.get_Tiempo_Desplazamiento_Mapa(lista_celdas.First(), lista_celdas), TipoAnimaciones.PERSONAJE));
