...
namespace Bot_Dofus_1._29._1.Otros.Entidades.Manejadores.Recolecciones
{
    public class Recoleccion : IDisposable
    {
        ...
        public bool get_Recolectar(List<short> ressourcesIds)
        {
            if (cuenta.esta_ocupado || interactivo_recolectando != null)
                return false;
            
            foreach (KeyValuePair<short, ObjetoInteractivo> kvp in get_Interactivos_Utilizables(ressourcesIds))
            {
        ...
        private bool get_Intentar_Mover_Interactivo(KeyValuePair<short, ObjetoInteractivo> interactivo)
        {
            ...
            ResultadoMovimientos resultado = cuenta.personaje.manejador.movimientos.get_Mover_A_Celda(interactivo.Key, true);
        ...
        {
            if (robado)
                evento_Recoleccion_Acabada(RecoleccionResultado.ROBADO);
            else
                cuenta.conexion.enviar_Paquete("GA500" + interactivo_recolectando.celda_id + ";" + interactivo_recolectando.modelo.habilidades[0]);
        }
        ...
        private void get_Movimiento_Finalizado(bool correcto)
        {
            ...
            if (!correcto && cuenta.personaje.manejador.movimientos.actual_path != null)
                evento_Recoleccion_Acabada(RecoleccionResultado.FALLO);
        }

        public async Task evento_Recoleccion_Iniciada(int id_personaje, int tiempo_delay, short celda_id, byte tipo_gkk)
        {
            if(interactivo_recolectando == null || interactivo_recolectando.celda_id != celda_id)
                return;
        ...
        {
            pathfinder.set_Mapa(cuenta.personaje.mapa);
